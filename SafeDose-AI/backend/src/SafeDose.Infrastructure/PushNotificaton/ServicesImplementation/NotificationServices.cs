    ...
    public class NotificationServices : INotificationServices
    {
        ...
        public async Task UserWillBeNotify()
        {
            ...
            foreach (var R in ReminderDate)
            {
                ...
                await SendAsync(R.AccountId, R.PatientMedicationTimeId, "حان وقت العلاج", R.DrugName, R.PatientName, R.PatientMedicationId,R.DrugTime.ToString());
            }

           // await SendAsync("fb02d9b6-1aba-431a-b095-e9372c89da03", 5, "testTitle", "testBody");
        }

        public async Task SendAsync(string AccountId, int PatientMedicationTimeId, string title, string body , string patientName , int PatientMedicationId,string DrugTime)
        {
            var subscriptions =
